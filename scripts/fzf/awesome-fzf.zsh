AWESOME_FZF_LOCATION="$HOME/awesome-fzf/awesome-fzf.zsh"
...
function zsurm() {
  if [[ "$#" -eq 0 ]]; then
    local files
    files=$(sudo find . -maxdepth 1 -type f | sudo fzf --multi)
    sudo echo $files | sudo xargs -I '{}' rm {} #we use xargs so that filenames to capture filenames with spaces in them properly
  else
    command sudo rm "$@"
  fi
}
