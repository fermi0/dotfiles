...
1. **Analyze** — Analyze vault structure manually or use available tools. Note md counts per
            folder vs last run (expected stable folders: Books, SQL, Study, Health, Phase3, Curious Cat, Scribble, AI, Research,
            Poetry, Excalidraw, Linux, reports).
2. **Plan** — if reorganization is warranted, generate organization plan manually, then
            present the dry-run diff to the user. Do NOT apply without explicit approval (approval → `reorganize_notes` with
            `create_backup=true, update_links=true`).
...
5. **Report** — write findings to `reports/maintenance/YYYY-MM-DD.md` in the vault (manually or via available tools),
            project folder `reports`), and append a one-line `chore` changelog entry there.
